The \texttt{llvmanim.cli} module is the user-facing entry point. It accepts LLVM~IR files and orchestrates the three pipeline layers via \texttt{argparse}; example usage is shown in CLI~\ref{cli:usage}.
\begin{minted}[
  autogobble,
  frame=single,
  bgcolor=listingbg,
...
\end{minted}
\vspace{-5ex}
\captionof{clilisting}{Basic usage}
\label{cli:usage}
