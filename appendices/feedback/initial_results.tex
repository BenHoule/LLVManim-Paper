...
% ADDRESSED - Updated figure
\feedback{Why are these [lines in figure 5] in a different color? The light gray makes it harder to read on a white background.}
{}
...
{
  We have not yet conducted any user testing, so we do not have any data on the effectiveness of the visualizations for users. We do not think speculation on this topic would be helpful at this stage.
}
...
% ADDRESSED - Updated package names
...
{
  The colors are primarily meant to help differentiate the layers quickly. While it is unfortunate that grayscale viewers lose out on this added benefit, it is not required for understanding the figures. However, we will consider adding symbols or patterns to further differentiate the layers in future iterations of the figures.
}
...
{
  We have provided a high level summary in introduction.
}
...
{
  We would need a more specific example of where this is an issue in order to address it, as the data types are mentioned throughout the architecture section and we are not sure which ones are not emphasized enough.
}
...
{
  Unforunately, page space is limited and we can only fit so many figures into the pages we have, resulting in some figures being pushed a page back due to how \LaTeX determines figure placement. However, we believe that the current placement does not significantly hinder understanding. We will consider adding brief definitions in the architecture section if we find that readers are having trouble understanding the diagrams without them.
}
...
{
  We have provided a usage example in the results section, and we will consider adding an example error scenario in future iterations of the report.
}
...
{
  This is a good idea, but we feel that it is outside the scope of this report, as it would require a significant amount of additional content to explain how to extend the scene graph or animation behavior. Such information would be better suited for the project's technical documentation (which we hope to include with the final report).
}
...
{
  The scene graph format is currently experimental, as we are still iterating on the design and may make changes based on user feedback and implementation experience. We will make sure to clearly indicate this in the documentation so that developers understand that they should not rely on the current format for long-term use until it is stabilized.
}
...
{
  Unfortunately, our group was not quite organized enough to warrant such a table. The existing timeline is already somewhat arbitrary in how it divides responsibilities, any additional division would be even more so.
}
...
{
   read carefully. tools dependency risks are discussed.
}
...
{
  The risks for this project are not easily quantifiable, so we chose to list them without ranking.
}
...
{
  At this point in the project, it is a bit late to update the timeline with specific integration milestones, as we have already completed a significant portion of the development work. However, we will ensure that integration testing is integrated into our ongoing development process, and we will consider adding more explicit milestones for these activities in future iterations of the project plan.
}
...
{
  We have already completed a significant portion of the development work, so it is a bit late to update the timeline with specific checkpoint deliverables.
}
...
{
  The paper is already crowded with figures, and we do not have the page space to include multiple versions of the same figure. We will consider adding simplified diagrams in future iterations of the report if we find that readers are having trouble understanding the current diagrams.
}
